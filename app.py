...
RENDER_EXTERNAL_URL = os.getenv("RENDER_EXTERNAL_URL")  # e.g. https://your-bot.onrender.com
...
"
    ...
    def order_line(order):
        ...
        return f"  `#{order['id']}` {order['service_name']} — *{order['price']}ج* ({time_str})
"
    ...
    def overview():
        ...
        text = Reports.header("📊 *نظرة عامة — اليوم*", f"📅 {today}
")
        text += f"💰 إجمالي الإيرادات: *{total} ج*
"
        text += f"📦 عدد الطلبات: *{count}*
...
"
        ...
        if cat_summary:
            for cat_key, stats in cat_summary.items():
                ...
                text += f"{label}
"
                text += f"  📦 {stats['count']} عدد | 💰 {stats['total']} ج
...
📌 *اختار:*"
    ...
    def daily():
        ...
        text = Reports.header("📊 *تقرير اليوم*", f"📅 {today}
")
        text += f"💰 الإجمالي: *{total} ج*
"
...
"
    ...
    def monthly():
        ...
        total = sum(o["price"] for o in orders)
        text = Reports.header(f"📈 *تقرير الشهر: {month}*", "")
        text += f"💰 الإجمالي: *{total} ج*
"
        text += f"📦 الطلبات: *{len(orders)}*
...
"
            for o in orders[-10:][::-1]:
                text += f"  `#{o['id']}` {o['service_name']} — {o['price']}ج ({o['date']})
"
        else:
            text += "📝 مفيش طلبات"
        return text
...
")
        if monthly:
            ...
            for month, stats in monthly.items():
                text += f"📆 `{month}`: *{stats['total']}ج* ({stats['count']} طلب)
"
            text += "━" * 18 + "
"
            text += f"💰 الإجمالي الكلي: *{grand_total}ج*
"
            text += f"📦 إجمالي الطلبات: *{grand_count}*"
        else:
            text += "📝 مفيش بيانات"
        return text
...
"
            f"🛠️ {service_name}
"
            f"السعر الافتراضي: *{default_price}ج*

"
            f"✅ اضغط تأكيد أو اكتب سعر جديد خلال 10 ثواني..."
        )
...
"
            f"هل أنت متأكد من حذف آخر طلب؟

"
            f"`#{last['id']}` {last['service_name']}
"
            f"💰 {last['price']}ج — {last['date']} {last['timestamp'][11:16]}"
        )
...
"
                f"اكتب رقم صحيح أو اضغط تأكيد

"
                f"🛠️ {service_name}
"
                f"السعر: *{pending['default_price']}ج*"
            )
...
"
                f"#{deleted['id']} {deleted['service_name']}
"
                f"💰 {deleted['price']}ج
...
"
            f"ملف: `{filename}`
"
            f"هيتم استبدال كل البيانات الحالية!
...
def run_flask():
    ...
    # For local development only — Render uses Gunicorn in production
